...
from diffusion_utils import (linear_beta_schedule, cosine_beta_schedule,
                              compute_diffusion_params, calculate_theoretical_snr)
...
def reverse_diffusion(model, y_t, start_step, params, device, save_intermediate=False):
    ...
    y = y_t.clone()
    start_step = min(start_step, len(betas) - 1)
    ...
    for t in tqdm(range(start_step, 0, -1), desc="Reverse diffusion"):
        ...
        # 获取参数
        alpha = alphas[t].view(-1, *([1] * (y.dim() - 1)))
        alpha_bar = alphas_cumprod[t].view(-1, *([1] * (y.dim() - 1)))
        alpha_bar_prev = alphas_cumprod_prev[t].view(-1, *([1] * (y.dim() - 1)))
        beta = betas[t].view(-1, *([1] * (y.dim() - 1)))
        ...
        # 采样
        if t > 1:
            variance = posterior_variance[t].view(-1, *([1] * (y.dim() - 1)))
...
def interpolate(model, sig1, sig2, interp_steps, params, device, n_interps=11):
    ...
    """
    alphas_cumprod = params['alphas_cumprod']
    interp_t = min(interp_steps, len(alphas_cumprod) - 1)
    alpha_bar = alphas_cumprod[interp_t].view(-1, *([1] * (sig1.dim() - 1)))
...
if __name__ == "__main__":
    ...
    # 显示理论 SNR
    interp_t = min(args.interp_steps, args.timesteps - 1)
    alpha_bar = params['alphas_cumprod'][interp_t]
    snr = calculate_theoretical_snr(alpha_bar)
    print(f"\n插值步数 {interp_t} 对应的理论 SNR: {snr:.2f} dB")
    if snr < 0:
